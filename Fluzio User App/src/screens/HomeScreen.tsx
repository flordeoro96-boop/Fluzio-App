  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        {/* Icon */}
        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#00E5FF] to-[#6C4BFF] flex items-center justify-center text-white flex-shrink-0">
          {icon}
        </div>
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onNavigate }) => {
  ...
  // Auto-show daily XP modal after 5 seconds if not claimed today
  useEffect(() => {
    if (userProfile && !dailyStreakClaimed && canClaimToday(userProfile.lastStreakRewardClaimed)) {
      const timer = setTimeout(() => {
        setShowXPClaimModal(true);
      }, 5000); // 5 seconds after login
      
      return () => clearTimeout(timer);
    }
  }, [userProfile, dailyStreakClaimed]);
  ...
  const loadProgression = async () => {
    ...
    try {
      ...
      // Load real recent activity from user's history
      const activities: any[] = [];
      
      try {
        // Get recent participations (missions) - no orderBy to avoid index requirement
        ...
        participationsSnap.forEach(doc => {
          ...
          activities.push({
            type: 'mission',
            title: data.missionTitle || 'Mission Completed',
            points: data.reward || 0,
            time: formatTimeAgo(data.createdAt),
            icon: Target
          });
        });

        // Get recent reward redemptions - no orderBy to avoid index requirement
        ...
        redemptionsSnap.forEach(doc => {
          ...
          activities.push({
            type: 'reward',
            title: `Redeemed ${data.rewardTitle}`,
            points: -(data.pointsCost || 0),
            time: formatTimeAgo(data.redeemedAt),
            icon: Gift
          });
        });

        // Get recent meetup participations - no orderBy to avoid index requirement
        ...
        meetupsSnap.forEach(doc => {
          const data = doc.data();
          activities.push({
            type: 'meetup',
            title: `${data.title} Meetup`,
            points: data.xpReward || 0,
            time: formatTimeAgo(data.startTime),
            icon: Users
          });
        });
        ...
        console.error('[HomeScreen] Error loading recent activity:', error);
        setRecentActivity([]);
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-[#F8F9FE] flex items-center justify-center">
        <div className="text-center">
          <Loader className="w-12 h-12 text-[#00E5FF] animate-spin mx-auto mb-4" />
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-[#00E5FF] to-[#6C4BFF] text-white p-6 rounded-b-3xl shadow-lg">
        <div className="flex items-start justify-between mb-4">
          ...
          {/* Daily Login Streak - Display Only */}
          <div
      ...
      {(userProfile?.role === 'MEMBER' || !userProfile?.role) && (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                      <div 
                        className="bg-gradient-to-r from-[#00E5FF] to-[#6C4BFF] h-full rounded-full"
                        style={{ width: '35%' }}
                      />
                    </div>
        ...
        {recentActivity.length > 0 && (
          <section>
            <div className="flex items-center gap-2 mb-3">
              <Sparkles className="w-5 h-5 text-[#FFB86C]" />
                ...
                <div key={idx} className="p-3 flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    ...
                    <activity.icon className={`w-5 h-5 ${
                      activity.type === 'mission' ? 'text-[#00E5FF]' :
                      activity.type === 'reward' ? 'text-[#6C4BFF]' :
                      activity.type === 'meetup' ? 'text-[#00E5FF]' :
                      'text-[#FFB86C]'
                    }`} />
                  </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-sm text-[#FFB86C] font-semibold hover:underline"
              >
                {t('common.viewAll')}
              </button>
            </div>
            
            <div className="bg-gradient-to-br from-[#FFB86C]/10 to-[#00E5FF]/10 rounded-lg p-4 border border-[#FFB86C]/20">
              <div className="flex gap-3 overflow-x-auto pb-2 -mx-2 px-2">
                {progression.badges.slice(0, 5).map((badge: any, idx: number) => (
                  <div key={idx} className="flex-shrink-0 text-center">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#FFB86C] to-[#00E5FF] flex items-center justify-center mb-2 shadow-lg">
        ...
        {events.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Calendar className="w-5 h-5 text-[#6C4BFF]" />
                <h2 className="text-lg font-bold text-[#1E0E62]">Today's Events</h2>
              </div>
              <button
                onClick={handleViewAllEvents}
                className="text-sm text-[#6C4BFF] font-semibold hover:underline"
              >
                View All
              </button>
            </div>
            ...
            {loadingEvents ? (
              <div className="flex items-center justify-center py-8">
                <Loader className="w-6 h-6 text-[#6C4BFF] animate-spin" />
              </div>
            ) : (
              <div className="overflow-x-auto -mx-4 px-4">
        ...
        {businesses.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-[#FFB86C]" />
            ...
            {loadingBusinesses ? (
              <div className="flex items-center justify-center py-8">
                <Loader className="w-6 h-6 text-[#FFB86C] animate-spin" />
              </div>
            ) : (
              <div className="overflow-x-auto -mx-4 px-4">
                <div className="flex space-x-3 pb-2">
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2 mb-2">
        <Calendar className="w-4 h-4 text-[#6C4BFF]" />
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-[#00E5FF]">{meetup.category}</span>
          </div>
...
const BusinessCard: React.FC<{
  ...
  return (
    ...
    >
      <div className="h-32 bg-gradient-to-br from-[#FFB86C] to-[#00E5FF]" />
      <div className="p-3">
