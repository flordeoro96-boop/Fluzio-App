...
export const applyToMission = async (
  ...
  try {
    ...
    // Get mission to check if it requires approval
    const { getMissionById } = await import('../../services/missionService');
    const mission = await getMissionById(missionId);
    
    // Check if this is a check-in mission (multiple ways to detect)
    const isCheckInMission = 
      mission?.triggerType === 'QR_SCAN' || 
      mission?.checkInMethod === 'QR_ONLY' ||
      mission?.checkInMethod === 'GPS' ||
      mission?.checkInMethod === 'BOTH' ||
      mission?.title?.includes('Visit') ||
      mission?.title?.includes('Check-In') ||
      mission?.title?.includes('Check In');
    
    // Check-in missions never require approval
    const requiresApproval = !isCheckInMission && mission?.requiresApproval !== false;
    
    console.log('[ParticipationService] Mission approval check:', {
      missionTitle: mission?.title,
      triggerType: mission?.triggerType,
      checkInMethod: mission?.checkInMethod,
      requiresApprovalField: mission?.requiresApproval,
      isCheckInMission,
      requiresApproval
    });
    
    // Create participation - auto-approve check-in missions
    const participationData = {
      ...
      ...(requiresApproval ? {} : { approvedAt: Timestamp.now() })
    };
    ...
    if (!requiresApproval && mission) {
      console.log('[ParticipationService] Auto-approving check-in mission, awarding points...');
      try {
        const { api } = await import('../../services/apiService');
        const userResult = await api.getUser(userId);
        
        console.log('[ParticipationService] User fetch result:', userResult.success, 'User points:', userResult.user?.points);
        
        if (userResult.success && userResult.user) {
          const pointsEarned = mission.reward?.points || 0;
          const currentPoints = userResult.user.points || 0;
          const newPoints = currentPoints + pointsEarned;
          
          console.log(`[ParticipationService] Awarding points: ${currentPoints} + ${pointsEarned} = ${newPoints}`);
          
          await api.updateUser(userId, { points: newPoints });
          console.log(`[ParticipationService] ✅ Auto-approved check-in: Awarded ${pointsEarned} points to user ${userId}`);
          
          // Log points transaction for tracking
          try {
            await logPointsTransaction(userId, 'EARN', pointsEarned, 'MISSION', `Check-in at ${mission.businessName || 'business'}`, currentPoints, newPoints);
          } catch (logError) {
            console.error('[ParticipationService] Failed to log points transaction:', logError);
          }
          ...
          await createNotification(userId, {
            ...
            message: `You earned ${pointsEarned} points for checking in at ${mission.businessName || 'the business'}!`,
            actionLink: `/missions/${missionId}`
          });
          console.log('[ParticipationService] Success notification sent to user');
        } else {
          console.error('[ParticipationService] Failed to fetch user for points award');
        }
      } catch (pointsError) {
        console.error('[ParticipationService] Failed to award points:', pointsError);
      }
    } else {
      console.log('[ParticipationService] Skipping auto-approval:', { requiresApproval, hasMission: !!mission });
    }
    
    // Send notification to business (only if requires approval)
    if (requiresApproval) {
      try {
        const { createNotification } = await import('../../services/notificationService');
        const { api } = await import('../../services/apiService');
  ...
  try {
    ...
    if (userSnap.exists()) {
      ...
      await updateDoc(userRef, {
        points: newPoints,
        level: newLevel,
        missionsCompleted: (userData.missionsCompleted || 0) + 1
      });
