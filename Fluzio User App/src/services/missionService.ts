...
const docToMission = (docData: DocumentData, docId: string): Mission => {
  ...
  return {
    ...
    createdAt: toISOString(docData.createdAt) || new Date().toISOString(),
    validUntil: toISOString(docData.validUntil || docData.expiresAt),
...
export const getMissionsForUser = async (user: UserProfile): Promise<Mission[]> => {
  try {
    // Fetch all active missions first, then filter by city in memory
    // This allows missions from businesses in the same city OR with matching businessCity
    const constraints: QueryConstraint[] = [
      where('lifecycleStatus', '==', 'ACTIVE'),
      where('isActive', '==', true)
    ];

    // Note: Removed creatorMode filter since all users are creators/influencers
    // All active missions are visible to all users
    ...
    const snapshot = await getDocs(q);
    const allMissions = snapshot.docs.map(doc => docToMission(doc.data(), doc.id));
    
    // Geographic filtering based on business type and subscription level
    return allMissions.filter(mission => isMissionVisibleToUser(mission, user));
  } catch (error) {
 ...
 * 
 * @param user - User profile with creator account type
...
export const getCreatorMissionsForUser = async (user: UserProfile): Promise<Mission[]> => {
  try {
    ...
    if (user.accountType !== 'creator' || !user.skills || user.skills.length === 0) {
      return [];
    }
...
export const createMission = async (data: Omit<Mission, 'id'>): Promise<{ success: boolean; missionId?: string; error?: string }> => {
  try {
    ...
    // Clean detailedRequirements to remove undefined values (Firestore doesn't allow them)
    const cleanDetailedRequirements: any = {};
    if (data.detailedRequirements) {
      if (data.detailedRequirements.postType) cleanDetailedRequirements.postType = data.detailedRequirements.postType;
      if (data.detailedRequirements.hashtags) cleanDetailedRequirements.hashtags = data.detailedRequirements.hashtags;
      if (data.detailedRequirements.mentions) cleanDetailedRequirements.mentions = data.detailedRequirements.mentions;
      if (data.detailedRequirements.minFollowers !== undefined) cleanDetailedRequirements.minFollowers = data.detailedRequirements.minFollowers;
    }
    
    // Clean reward object to remove undefined values
    const cleanReward: any = {
      type: data.reward?.type || 'POINTS_ONLY',
      points: data.reward?.points || data.points || 0
    };
    if (data.reward?.itemDescription) {
      cleanReward.itemDescription = data.reward.itemDescription;
    }
    ...
    // Only add fields if they have values (not undefined/null)
    if (Object.keys(cleanDetailedRequirements).length > 0) {
      missionData.detailedRequirements = cleanDetailedRequirements;
    }
    if (data.location) missionData.location = data.location;
    if (data.geo) missionData.geo = data.geo;
    if (data.radiusMeters) missionData.radiusMeters = data.radiusMeters;
    if (data.goal) missionData.goal = data.goal;
    if (data.image || data.imageUrl) missionData.image = data.image || data.imageUrl;
    
    // Handle validUntil - convert to Timestamp if string, keep if already Timestamp
    if (data.validUntil) {
      if (typeof data.validUntil === 'string') {
        missionData.validUntil = Timestamp.fromDate(new Date(data.validUntil));
      } else {
        missionData.validUntil = data.validUntil;
      }
    } else if (data.expiresAt) {
      missionData.validUntil = Timestamp.fromDate(new Date(data.expiresAt));
    }
    
    if (data.qrCodeSecret) missionData.qrCodeSecret = data.qrCodeSecret;
    if (data.approvalRequired !== undefined) missionData.approvalRequired = data.approvalRequired;
    if (data.autoApprove !== undefined) missionData.autoApprove = data.autoApprove;
    if (data.budget) missionData.budget = data.budget;
    if (data.targetAudience) missionData.targetAudience = data.targetAudience;
    if (data.targetLevel) missionData.targetLevel = data.targetLevel;
    if (data.targetCategories) missionData.targetCategories = data.targetCategories;
    if (data.isStandard !== undefined) missionData.isStandard = data.isStandard;
