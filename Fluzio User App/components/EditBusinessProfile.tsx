...
export const EditBusinessProfile: React.FC<EditBusinessProfileProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    checkInVerificationMethod: 'QR_ONLY' as 'QR_ONLY' | 'GPS' | 'BOTH',
    bio: '',
    mission: '',
    photoUrl: '',
    website: '',
  ...
  useEffect(() => {
    if (business || userProfile) {
      ...
      setFormData({
        ...
        checkInVerificationMethod: (source.checkInVerificationMethod || business?.checkInVerificationMethod || 'QR_ONLY') as 'QR_ONLY' | 'GPS' | 'BOTH',
  ...
  const handleConnectGoogle = async () => {
    console.log('[EditBusinessProfile] 🔵 Starting Google connection...');
    try {
      setIsLoading(true);
      console.log('[EditBusinessProfile] 🔵 Calling socialAuthService.linkGoogle()...');
      const result = await socialAuthService.linkGoogle();
      
      console.log('[EditBusinessProfile] 🔵 Google link result:', result);
      
      if (result.success) {
        console.log('[EditBusinessProfile] ✅ Google account connected successfully');
        alert('✅ Google account connected successfully!');
        
        // Try to get location from device when connecting Google
        if (navigator.geolocation) {
          console.log('[EditBusinessProfile] 📍 Attempting to get geolocation...');
          navigator.geolocation.getCurrentPosition(
            (position) => {
              console.log('[EditBusinessProfile] 📍 Location detected:', {
            ...
            (error) => {
              console.log('[EditBusinessProfile] ⚠️ Could not auto-detect location:', error);
        ...
        console.log('[EditBusinessProfile] 🔄 Refreshing user profile...');
        await refreshUserProfile(); // Refresh to show updated connection
        
        // Open Google Business Sync modal
        console.log('[EditBusinessProfile] 🚀 Opening Google Business Sync modal...');
        setShowGoogleSync(true);
      } else {
        console.error('[EditBusinessProfile] ❌ Google connection failed:', result.error);
        alert(result.error || 'Failed to connect Google account');
      }
    } catch (error: any) {
      console.error('[EditBusinessProfile] ❌ Google connection error:', error);
  ...
  const handleDisconnectGoogle = async () => {
    console.log('[EditBusinessProfile] 🔵 Disconnecting Google...');
    const confirmed = confirm(t('businessEdit.confirmDisconnect', { provider: 'Google' }));
    if (!confirmed) return;
    
    try {
      setIsLoading(true);
      console.log('[EditBusinessProfile] 🔵 Calling socialAuthService.unlinkGoogle()...');
      const result = await socialAuthService.unlinkGoogle();
      
      console.log('[EditBusinessProfile] 🔵 Google unlink result:', result);
      
      if (result.success) {
        console.log('[EditBusinessProfile] ✅ Google disconnected successfully');
        alert(t('businessEdit.disconnectSuccess', { provider: 'Google' }));
        await refreshUserProfile(); // Refresh to show updated connection
      } else {
        console.error('[EditBusinessProfile] ❌ Disconnect failed:', result.error);
        alert(result.error || t('businessEdit.disconnectFailed', { provider: 'Google' }));
      }
    } catch (error: any) {
      console.error('[EditBusinessProfile] ❌ Google disconnect error:', error);
  ...
  const handleGoogleSyncComplete = async (data: any) => {
    console.log('[EditBusinessProfile] ✅ Google Business sync completed!', data);
    console.log('[EditBusinessProfile] 📊 Synced data:', {
      businessName: data.googleBusinessName,
      address: data.address,
      phone: data.phone,
      website: data.website,
      rating: data.rating,
      reviewCount: data.reviewCount,
      attributesCount: data.googleAttributes?.length || 0,
      photosCount: data.googlePhotos?.length || 0
    });
    
    // Update form with synced data
    if (data.phone) {
      console.log('[EditBusinessProfile] 📞 Updating phone:', data.phone);
      setFormData(prev => ({ ...prev, phone: data.phone }));
    }
    
    if (data.website) {
      console.log('[EditBusinessProfile] 🌐 Updating website:', data.website);
      setFormData(prev => ({ ...prev, website: data.website }));
    }
    
    // Refresh profile to show all synced data
    console.log('[EditBusinessProfile] 🔄 Refreshing user profile with synced data...');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await api.updateUser(userId, {
        ...
        checkInVerificationMethod: formData.checkInVerificationMethod,
        bio: formData.bio,
  ...
  return (
    <div className="fixed inset-0 z-[90] bg-white flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="max-w-2xl mx-auto space-y-6 pb-20">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              {(formData.businessMode === 'PHYSICAL' || formData.businessMode === 'HYBRID') && (
                <div>
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
                    <p className="text-sm text-blue-900 font-semibold mb-2">ℹ️ Check-In Verification Method</p>
                    <p className="text-xs text-blue-700">
                      The check-in verification method is now configured per mission when you activate "Visit & Check-In" missions. 
                      Go to <strong>Missions → In-Person & Visits</strong> to activate and choose your preferred verification method (QR Code, GPS, or Both).
                    </p>
                  </div>
                  
                  <Select 
                    label="Default Check-In Verification (Legacy)" 
                    value={formData.checkInVerificationMethod} 
                    onChange={e => setFormData({...formData, checkInVerificationMethod: e.target.value as 'QR_ONLY' | 'GPS' | 'BOTH'})}
                    disabled
                  >
                    <option value="QR_ONLY">QR Code Only (Recommended)</option>
                    <option value="GPS">GPS Only</option>
                    <option value="BOTH">QR Code or GPS</option>
                  </Select>
                  <p className="text-xs text-gray-400 mt-1 italic">
                    This setting is deprecated. Each mission activation now has its own verification method.
                  </p>
                </div>
              )}
                  ...
                  <div>
                    ...
                    {business.lastGoogleSync && (
                      <div className="text-xs text-blue-600 mt-1">
                        Last synced: {new Date(business.lastGoogleSync).toLocaleDateString()}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <Card className="p-6">
            <LocationManager 
              ...
              onUpdateLocations={async (locations) => {
                try {
                  // Update user profile with new locations
                  await api.updateUser(business.id, { 
                    locations: locations 
                  });
            ...
            <div className="space-y-2">
              {OFFER_OPTIONS.map(offer => (
                <label 
                  ...
                  style={{ borderColor: formData.offers.includes(offer) ? '#6C4BFF' : '#E5E7EB' }}
                >
                  <input
                    type="checkbox"
      ...
      <GoogleBusinessSync
        ...
        onClose={() => {
          console.log('[EditBusinessProfile] 🔵 Closing Google Business Sync modal');
