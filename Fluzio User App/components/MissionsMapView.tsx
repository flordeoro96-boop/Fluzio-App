...
export const MissionsMapView: React.FC<MissionsMapViewProps> = ({
  ...
  const sortedMissions = [...enrichedMissions].sort((a, b) => {
    switch(sortBy) {
      ...
      case 'expiry':
        const aExpiry = a.expiresAt ? a.expiresAt.getTime() : Date.now() + 999999;
        const bExpiry = b.expiresAt ? b.expiresAt.getTime() : Date.now() + 999999;
  ...
  useEffect(() => {
    ...
    enrichedMissions.forEach(mission => {
      ...
      const isUrgent = mission.expiresAt && mission.expiresAt.getTime() - Date.now() < 24 * 60 * 60 * 1000;
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      ...
                      {mission.expiresAt && (
                        <span className="flex items-center gap-1 text-red-500">
                          ...
                          {mission.expiresAt.toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  </div>
