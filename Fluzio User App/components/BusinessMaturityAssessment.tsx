  ...
  const handleAnswer = (value: string) => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      setTimeout(() => {
        ...
        // Update level first
        onUpdate('calculatedLevel', level);
...
function calculateBusinessLevel(data: OnboardingState): 'FREE' | 'SILVER' | 'GOLD' | 'PLATINUM' {
  ...
  score += stageScores[data.businessStage || 'validating'] || 0;
  ...
  score += ageScores[data.businessAge || 'not_launched'] || 0;
  ...
  score += customerScores[data.customerBase || 'none'] || 0;
  ...
  score += revenueScores[data.monthlyRevenue || 'none'] || 0;
  ...
  score += teamScores[data.teamSize || 'solo'] || 0;
  ...
  score += presenceScores[data.onlinePresence || 'none'] || 0;
  
  // Total possible: 31 points
  // Apply special rules
  
  // If business age < 6 months → max SILVER
  if (data.businessAge === 'not_launched' || data.businessAge === 'under_6m') {
    if (score >= 8) return 'SILVER';
    return 'FREE';
  }
  
  // If revenue > €50k → min GOLD
  if (data.monthlyRevenue === '50k_200k' || data.monthlyRevenue === '200k_plus') {
    return 'PLATINUM';
  }
  
  // If team > 20 → PLATINUM
  if (data.teamSize === '20_plus') {
    return 'PLATINUM';
  }
