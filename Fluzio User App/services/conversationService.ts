...
export const deleteConversation = async (conversationId: string): Promise<void> => {
  try {
    // Only delete the conversation document
    // Messages will be orphaned but can be cleaned up by a Cloud Function later
    // This avoids permission issues when trying to delete other users' messages
...
export const getOrCreateProjectConversation = async (
  ...
  try {
    ...
    const participants = [leadBusinessId];
    const participantNames: Record<string, string> = {
      [leadBusinessId]: leadBusinessName
    };
    const participantAvatars: Record<string, string> = {};
    const participantRoles: Record<string, string> = {
      [leadBusinessId]: 'business'
    };

    if (leadBusinessAvatar) {
      participantAvatars[leadBusinessId] = leadBusinessAvatar;
    }

    // Add accepted creators as participants
    if (acceptedCreators && acceptedCreators.length > 0) {
      acceptedCreators.forEach(creator => {
        participants.push(creator.id);
        participantNames[creator.id] = creator.name;
        participantRoles[creator.id] = 'creator';
        if (creator.avatar) {
          participantAvatars[creator.id] = creator.avatar;
        }
      });
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log('[ConversationService] Created project conversation:', docRef.id);
    }
