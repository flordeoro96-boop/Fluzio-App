...
class SocialAuthService {
  ...
  async linkGoogle(): Promise<SocialLinkResult> {
    try {
      ...
      // Store OAuth access token for Google Business Profile API
      const accessToken = credential.accessToken;
      console.log('[SocialAuth] 🔑 Access token received:', !!accessToken);
      console.log('[SocialAuth] 🔑 Access token length:', accessToken?.length || 0);
      
      if (accessToken) {
        console.log('[SocialAuth] 🔵 Storing access token in googleBusinessService...');
        const { googleBusinessService } = await import('./googleBusinessService');
        googleBusinessService.setAccessToken(accessToken);
        console.log('[SocialAuth] ✅ Access token stored in memory');
      } else {
        console.warn('[SocialAuth] ⚠️ No access token in credential');
      }

      console.log('[SocialAuth] Google linked successfully:', { displayName, email, providerUserId });

      // Update backend with Google connection (including access token)
      console.log('[SocialAuth] 🔵 Updating Firestore with social account data...');
      await this.updateSocialAccount('google', {
        ...
        displayName,
        photoURL,
        accessToken: accessToken || undefined // Store for future API calls
      });
      console.log('[SocialAuth] ✅ Firestore updated with token');
  ...
  /**
   * Refresh Google access token (re-authenticate to get fresh token)
   */
  async refreshGoogleToken(): Promise<{ success: boolean; accessToken?: string; error?: string }> {
    try {
      const currentUser = auth.currentUser;
      if (!currentUser) {
      ...
      console.log('[SocialAuth] Refreshing Google access token...');
      ...
      try {
        // Try reauthentication with popup
        ...
        const { googleBusinessService } = await import('./googleBusinessService');
        googleBusinessService.setAccessToken(credential.accessToken);

        console.log('[SocialAuth] ✅ Google token refreshed successfully');
        return { success: true, accessToken: credential.accessToken };
      } catch (popupError: any) {
        // If silent refresh fails, require user interaction
        if (popupError.code === 'auth/popup-closed-by-user' || popupError.code === 'auth/cancelled-popup-request') {
          console.log('[SocialAuth] User cancelled token refresh');
          return { success: false, error: 'Token refresh cancelled by user' };
        }
        throw popupError;
      }
    } catch (error: any) {
      console.error('[SocialAuth] Token refresh error:', error);
      return { 
        success: false, 
