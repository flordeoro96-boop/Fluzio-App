 ...
 * NOTE: This should only be called for CUSTOMER users, not BUSINESS users
 * Business locations are fixed to their address and should not auto-update
 */
export async function updateUserLocation(userId: string, userRole?: string): Promise<GeoPoint | null> {
  // Skip location updates for business users - their location is their fixed address
  ...
  if (location) {
    // Update user in Firestore (only for customers)
    try {
 ...
 * For businesses, returns their fixed address location
 * For customers, returns current/cached location
 */
export async function getUserLocation(userId: string, userRole?: string, maxAgeMinutes: number = 30): Promise<GeoPoint | null> {
  // For business users, don't track live location
  if (userRole === 'BUSINESS') {
    return null; // Business location comes from their profile's fixed address
  }
  ...
  // Fetch fresh location (only for customers)
  return await updateUserLocation(userId, userRole);
}

/**
 * Watch user location for continuous updates
 * Business users are skipped - their location is their fixed address
 */
export function watchUserLocation(
  ...
  // Skip location watching for business users - their location is their fixed address
  if (userRole === 'BUSINESS') {
    console.log('[locationService] Skipping location watch for business user');
    return null;
  }
  ...
  return navigator.geolocation.watchPosition(
    async (position) => {
      ...
      // Update user in Firestore (only for customer users)
      try {
        await api.updateUser(userId, {
          geo: geoPoint,
