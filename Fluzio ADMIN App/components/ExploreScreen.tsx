      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-4">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {biz.missionCount && biz.missionCount > 0 && (
              <div className="bg-gradient-to-r from-[#FFC300] to-[#FF8C00] px-2.5 py-1 rounded-lg text-xs font-bold text-white shadow-lg flex items-center gap-1">
        ...
        <div className="flex items-center justify-between mt-1 gap-2">
          <div className="flex-1 min-w-0">
            ...
            {(() => {
              const status = isBusinessOpen(biz.openingHours as any);
              if (status) {
                return (
...
export const ExploreScreen: React.FC<{ user: User }> = ({ user }) => {
    ...
    useEffect(() => {
        const loadBusinesses = async () => {
            ...
            try {
                ...
                const businesses: User[] = firestoreBusinesses.map(b => ({
                    ...b,
                ...
                setAllBusinesses(store.getUsers().filter(u => u.role === UserRole.BUSINESS));
            } finally {
    ...
    if (showOnlyOpen) {
        filteredPhysical = filteredPhysical.filter(b => {
            const status = isBusinessOpen(b.openingHours as any);
    ...
    const businesses = location 
        ? filteredPhysical
            .map(biz => {
                ...
                const totalPoints = bizMissions.reduce((sum, m) => sum + ((m as any).points || m.pointsReward || 0), 0);
                return {
                    ...biz,
    ...
    filteredOnline = filteredOnline.filter(business => {
        ...
        if (subscription === 'PLATINUM' || subscription === 'GOLD') {
            return true;
        }
    ...
    useEffect(() => {
      if (view === 'MAP' && mapContainerRef.current && !mapInstanceRef.current && window.L && !loadingBusinesses) {
          ...
          const createIcon = (isActive: boolean, avatarUrl: string) => {
              ...
              const borderClass = isActive ? 'border-[#F72585]' : 'border-white';
              
              return window.L.divIcon({
                  className: 'custom-pin',
                  html: `
                    <div class="relative flex items-center justify-center w-10 h-10">
                        ${isActive ? `<div class="absolute inset-0 bg-[#F72585] rounded-full opacity-50 ${pulseClass}"></div>` : ''}
          ...
          const markers = window.L.markerClusterGroup({
              ...
              iconCreateFunction: function(cluster: any) {
                  ...
                  return window.L.divIcon({
                      html: `<div class="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-[#F72585] to-[#7209B7] text-white font-bold text-sm shadow-xl border-2 border-white">${count}</div>`,
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-9 pr-8 py-2 bg-white/80 rounded-2xl text-sm text-gray-800 outline-none border border-gray-200 focus:border-[#F72585] transition-all"
                            />
                            {searchQuery && (
                                <button 
                                    onClick={() => setSearchQuery('')}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#F72585] transition-colors"
                                >
                                    ✕
                                </button>
                            )}
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                ...
                                {activeFiltersCount > 0 && (
                                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-gradient-to-r from-[#F72585] to-[#7209B7] text-white text-[10px] rounded-full flex items-center justify-center animate-pulse">
                                        {activeFiltersCount}
                                    </span>
                                )}
                            </button>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200 space-y-3 animate-in slide-in-from-top duration-200">
                                        ...
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-2xl text-xs font-semibold transition-all border active:scale-95 ${
                                                selectedCategory === cat.key
                                                    ? 'bg-gradient-to-r from-[#F72585] to-[#7209B7] text-white border-transparent shadow-md'
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    style={{
                                        background: `linear-gradient(to right, #F72585 0%, #F72585 ${maxDistance}%, #E2E6EA ${maxDistance}%, #E2E6EA 100%)`
                                    }}
                                />
                            </div>
                         ...
                         {loadingBusinesses && (
                             <div className="absolute inset-0 z-10 bg-white/90 flex items-center justify-center">
                                 <div className="text-center">
                                     <div className="animate-spin w-12 h-12 border-4 border-[#F72585] border-t-transparent rounded-full mx-auto mb-4"></div>
                     ...
                     <div className="p-6 pt-32 space-y-6 overflow-y-auto h-full bg-white">
                         ...
                         {onlineShops.length > 0 && (
                             <>
                                 <div className="flex items-center justify-between">
                                     ...
                                     <span className="text-xs font-bold text-[#F72585] bg-[#F72585]/10 px-3 py-1.5 rounded-full">{t('explore.nationwide')}</span>
                                 </div>
                                 <div className="grid grid-cols-2 gap-4">
                                     {onlineShops.map((biz) => (
                                         <div key={biz.id} className="aspect-[4/5] rounded-2xl bg-gray-100 relative overflow-hidden group cursor-pointer" onClick={() => setViewingBusinessId(biz.id)}>
                                             ...
                                             />
                                             <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-4">
                                                 <div className="absolute top-3 right-3 bg-gradient-to-r from-[#F72585] to-[#7209B7] px-2 py-1 rounded-lg text-xs font-bold text-white">
                                                     {t('missions.online')}
                                                 </div>
                                                 <span className="text-white font-bold text-sm line-clamp-1">{biz.name}</span>
                         ...
                         {!loadingBusinesses && businesses.length === 0 && onlineShops.length === 0 && (
                             <div className="text-center py-12 px-6">
                                 <div className="w-20 h-20 bg-gradient-to-br from-[#F72585]/20 to-[#7209B7]/20 rounded-full mx-auto mb-6 flex items-center justify-center">
                                     <Search className="w-10 h-10 text-[#F72585]" />
                                 </div>
                                 ...
                                 <div className="space-y-3">
                                     {searchQuery && (
                                         <button 
                                             onClick={() => setSearchQuery('')}
                                             className="w-full px-4 py-3 bg-gradient-to-r from-[#F72585] to-[#7209B7] text-white rounded-xl font-bold hover:shadow-lg transition-all"
                                         >
                                     ...
                                     {selectedCategory !== 'All' && (
                                         <button 
                                             ...
                                             className="w-full px-4 py-3 bg-white border-2 border-[#F72585] text-[#F72585] rounded-xl font-bold hover:bg-[#F72585] hover:text-white transition-all"
                                         >
                                     ...
                                     {maxDistance < 100 && (
                                         <button 
                                             ...
                                             className="w-full px-4 py-3 bg-white border-2 border-gray-200 text-[#1E0E62] rounded-xl font-bold hover:border-[#F72585] transition-all"
                                         >
