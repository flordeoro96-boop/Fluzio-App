...
export const MissionCard: React.FC<MissionCardProps> = ({ 
  ...
  const isExpired = (mission as any).expiresAt ? new Date((mission as any).expiresAt) < new Date() : false;
  const isCompleted = isApplied && (mission as any).status === 'COMPLETED';
  const daysRemaining = (mission as any).expiresAt 
    ? Math.ceil((new Date((mission as any).expiresAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
    : null;
    ...
    <Card className="overflow-hidden transition-all hover:shadow-lg">
      ...
      <div className="p-4 pb-3 flex items-center gap-3">
        {mission.businessLogo ? (
          ...
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#7209B7] to-[#F72585] flex items-center justify-center text-white font-bold shadow-sm">
        ...
        <div className="mx-4 mb-4 p-3 bg-gray-50 rounded-xl">
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-[#7209B7]">{stats.completed}</div>
            </div>
            <div>
        ...
        <div className="mx-4 mb-4 flex items-center justify-between p-3 bg-white rounded-xl border">
          ...
          <button 
            ...
            className={`w-12 h-7 rounded-full transition-colors p-1 flex items-center ${
              mission.isActive || mission.lifecycleStatus === 'ACTIVE' ? 'bg-[#F72585]' : 'bg-gray-300'
