    ...
    <div className="space-y-6">
      ...
      <div>
        <SquadActivityPlanner
          ...
          onSelectActivity={(activity, meetupType) => {
            console.log('Selected activity:', activity, 'Type:', meetupType);
            
            // Create or get squad chat
            if (squad) {
              const chatId = getOrCreateSquadChat(
                squad.id,
                squad.members,
                squad.name || `Squad ${squad.month}`
              );
              
              // Propose activity in chat
              const proposalId = proposeActivity(chatId, currentUser.id, {
                ...activity,
                meetupType
              });
