        ...
        <AIEventGeneratorModal
          ...
          onUseIdea={(idea, context) => {
            ...
            setEditingEvent({
              ...
              pricing: { 
                pointsCost: idea.pricingGuidance?.pointsCost, 
                moneyCost: idea.pricingGuidance?.moneyCost, 
                currency: 'EUR' 
              },
              status: 'DRAFT',
              createdBy: adminId,
          ...
          {event && !event.id && (
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-lg p-3 flex items-center gap-2">
              ...
              <p className="text-sm text-purple-900">
                <strong>AI-Generated Event</strong> - Review and customize the pre-filled details below before saving.
              </p>
            </div>
          )}
...
const AIEventGeneratorModal: React.FC<{
  ...
  const handleGenerate = async () => {
    ...
    try {
      console.log('🎨 Generating AI event ideas with params:', {
        duration,
        categories,
        targetAudience,
        genderRestriction,
      ...
      const generatedIdeas = await generateEventIdeas({
        duration,
        categories: categories.length > 0 ? categories : ['networking', 'business'],
        targetAudience,
        genderRestriction,
        location,
        locationName,
      ...
      console.log('✅ Generated', generatedIdeas.length, 'event ideas');
      setIdeas(generatedIdeas);

      if (generatedIdeas.length === 0) {
        alert('No ideas generated. Please try adjusting your parameters.');
      }
    } catch (error) {
  ...
  const handleUseIdea = (idea: any) => {
    console.log('📋 Using AI-generated idea:', idea.title);
    onUseIdea(idea, {
      targetAudience,
    ...
    alert(`✨ Event idea applied!\n\nThe event form has been pre-filled with:\n• ${idea.title}\n• ${idea.estimatedAttendees} attendees\n• Location: ${idea.suggestedLocation}\n• Pricing: ${idea.pricingGuidance?.pointsCost} points / €${idea.pricingGuidance?.moneyCost}\n\nYou can now review and customize the details before saving.`);
  };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-bold text-[#1E0E62] mb-1">AI-Powered Event Generation</h3>
                <p className="text-sm text-[#8F8FA3]">
                  Configure your event parameters below and let AI generate 5 creative, detailed event concepts.
                  Each includes title, description, activities, location suggestions, attendee estimates, and pricing guidance.
                  Click "Use This Idea" to auto-fill the event creation form.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-[#1E0E62] flex items-center gap-2">
                  ...
                  {ideas.length} AI-Generated Event Ideas
                </h3>
                <button
                  onClick={handleGenerate}
                  disabled={generating}
                  className="flex items-center gap-2 px-4 py-2 border-2 border-purple-600 text-purple-600 rounded-lg font-semibold hover:bg-purple-50 transition-all disabled:opacity-50"
                >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-[#1E0E62] font-medium">
                            {idea.pricingGuidance?.pointsCost} pts / €{idea.pricingGuidance?.moneyCost}
                          </span>
                        </div>
                      </div>
