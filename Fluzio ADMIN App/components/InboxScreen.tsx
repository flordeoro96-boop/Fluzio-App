...
export const InboxScreen: React.FC<InboxScreenProps> = ({ isOpen, onClose, user, initialConversationId }) => {
  ...
  const mockConversations = store.getConversationsForUser(user.id);
  
  // Combine mock conversations and Firestore conversations with deduplication
  // Prioritize Firestore conversations over mock ones (Firestore is source of truth)
  const conversations = React.useMemo(() => {
    const firestoreIds = new Set(firestoreConversations.map(c => c.id));
    const uniqueMockConversations = mockConversations.filter(c => !firestoreIds.has(c.id));
    const combined = [...firestoreConversations, ...uniqueMockConversations];
    console.log('[InboxScreen] 🔄 Conversations memo updated:', {
      firestoreCount: firestoreConversations.length,
      mockCount: uniqueMockConversations.length,
      totalCount: combined.length,
      conversationIds: combined.map(c => ({ id: c.id, type: c.type, name: c.name }))
    });
    return combined;
  }, [mockConversations, firestoreConversations]);
  ...
  const handleCloseChat = () => {
      ...
      // If we came from deep link, maybe close entirely or just go back to list?
      // For now, going back to list is safer UI flow.
  };
  ...
  const filteredConversations = conversations.filter(c => {
      ...
      if (activeTab === 'PARTNERS') {
          matchesTab = c.type === 'SQUAD_GROUP' || c.type === 'B2B_DM';
      } else {
          matchesTab = c.type === 'AMBASSADOR_DM';
      }
  ...
  const businessesUnreadCount = conversations
    .filter(c => c.type === 'SQUAD_GROUP' || c.type === 'B2B_DM')
    .reduce((total, c) => total + (c.unreadCount || 0), 0);
    ...
    <div className="fixed inset-0 z-[150] bg-[#F8F9FE] flex flex-col animate-in slide-in-from-right duration-300 font-sans text-[#1E0E62]">
            ...
            <div className="h-20 flex items-center px-4 gap-4">
                ...
                {firestoreConversations.length > 0 && (
                  <button 
                    ...
                    className="px-3 py-1.5 text-xs font-medium text-[#7209B7] hover:bg-[#7209B7]/10 rounded-lg transition-colors disabled:opacity-50"
                ...
                <button 
                  ...
                  className="p-3 bg-gradient-to-r from-[#F72585] to-[#7209B7] text-white rounded-full hover:shadow-lg transition-all active:scale-95"
                >
            ...
            <div className="px-4 pb-4">
                <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-10 py-2.5 bg-white rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#7209B7] focus:ring-2 focus:ring-[#7209B7]/20 transition-all"
                    />
                    {searchTerm && (
                        <button
        ...
        <div className="p-4 bg-white/50 shrink-0">
            <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-white">
                 ...
                 >
                    ...
                    {t('inbox.tabs.individuals', { defaultValue: 'Individuals' })}
        ...
        <div className="px-4 pb-2">
            <div className="relative group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#8F8FA3] group-focus-within:text-[#F72585] transition-colors" />
                <input 
                  type="text" 
                  placeholder={t('messages.searchMessages')} 
                  className="w-full pl-12 pr-4 py-3.5 bg-white rounded-2xl border border-white text-sm font-medium focus:ring-2 focus:ring-[#F72585]/20 focus:border-[#F72585] outline-none transition-all placeholder:text-[#8F8FA3] shadow-[0_10px_40px_rgba(114,9,183,0.05)] text-[#1E0E62]"
                />
            </div>
        </div>
...
const InboxItem: React.FC<{ conversation: Conversation, currentUser: User, onClick: () => void, onDelete?: (id: string) => void }> = ({ conversation, currentUser, onClick, onDelete }) => {
  ...
  const { t } = useTranslation();
    ...
    if (conversation.type === 'SQUAD_GROUP') {
        ...
        avatarDisplay = (
            <div className="w-14 h-14 bg-gradient-to-br from-[#FFC300] to-[#F72585] text-white rounded-2xl flex items-center justify-center shadow-lg shadow-[#F72585]/20">
        ...
        avatarDisplay = (
            <div className="relative">
                ...
                />
                {conversation.unreadCount > 0 && <div className="absolute -top-1 -right-1 w-4 h-4 bg-[#F72585] rounded-full border-2 border-white"></div>}
            </div>
        );
    }

    const highlightClass = 'bg-white/80 border-transparent hover:border-white shadow-[0_4px_20px_rgba(114,9,183,0.02)]';
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {conversation.contextTag && (
                        <div className="text-[10px] font-extrabold text-[#7209B7] bg-[#7209B7]/5 px-2 py-0.5 rounded-full inline-block mb-1.5 tracking-wide uppercase">
                            {conversation.contextTag}
                        </div>
                    )}
                ...
                {conversation.unreadCount > 0 && (
                    <div className="w-6 h-6 bg-[#F72585] text-white text-xs font-bold rounded-full flex items-center justify-center shrink-0 shadow-lg shadow-[#F72585]/30">
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col">
        ...
        {isGroupMode && (
          <div className="p-4 border-b border-gray-100">
            <input
              ...
              className="w-full px-4 py-3 bg-[#F8F9FE] rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#F72585]/20 outline-none placeholder:text-[#8F8FA3] text-[#1E0E62]"
            />
          </div>
        )}
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="relative">
            ...
            <input
              ...
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-4 py-3 bg-[#F8F9FE] rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#F72585]/20 focus:border-[#F72585] outline-none transition-all placeholder:text-[#8F8FA3] text-[#1E0E62]"
              autoFocus
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {searching ? (
            ...
            <div className="text-center py-10">
              <div className="w-16 h-16 bg-gradient-to-br from-[#F72585] to-[#7209B7] rounded-full flex items-center justify-center mx-auto mb-4 text-white">
        ...
        {isGroupMode && selectedUsers.length >= 2 && (
          <div className="p-4 border-t border-gray-100">
            <button
              ...
              className="w-full py-3 bg-gradient-to-r from-[#F72585] to-[#7209B7] text-white font-bold rounded-xl hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {creating ? (
